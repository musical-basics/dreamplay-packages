import { z } from "zod";
import { createEmailSupabaseClient, emailError, emailJson, readJson, requireAgentAuth, } from "./database.js";
import { sendEmailCampaign } from "./send-server.js";
import { EMAIL_WORKSPACES, normalizeEmail, normalizeTags, } from "./schema.js";
const jsonValue = z.lazy(() => z.union([z.string(), z.number(), z.boolean(), z.null(), z.array(jsonValue), z.record(jsonValue)]));
...
export function createEmailAgentHandler(options = {}) {
    return async function handleEmailAgentRequest(request, context) {
        try {
...
export function createEmailEditorHandler(options = {}) {
    const agentHandler = createEmailAgentHandler(options);
    return async function handleEmailEditorRequest(request, context) {
...
async function handleCampaigns(request, method, workspace, path, options) {
    ...
    if (method === "GET" && !campaignId) {
        ...
        if (error)
            return emailError(error.message, 500);
    ...
    if (method === "GET" && campaignId && !action) {
        ...
        if (error)
            return emailError(error.message, 500);
        if (!data)
            return emailError("Campaign not found", 404);
    ...
    if (method === "POST" && campaignId && action === "clone") {
        ...
        const { data: source, error: sourceError } = await supabase
            ...
            .maybeSingle();
        if (sourceError)
            return emailError(sourceError.message, 500);
        if (!source)
            return emailError("Campaign not found", 404);
        const sourceVars = asJsonObject(source.variable_values);
        ...
        const { data, error } = await supabase
            .from("campaigns")
            .insert({
            name: parsed.data.name ?? `${source.name ?? "Campaign"} (Clone ${new Date().toLocaleDateString()})`,
            ...
            workspace,
            email_type: source.email_type || "campaign",
...
async function handleSubscribers(request, method, workspace, path, options) {
    const supabase = createEmailSupabaseClient(options);
    ...
    if (method === "GET" && !subscriberId) {
        ...
        for (const notTag of notTags)
            query = query.not("tags", "cs", `{${notTag}}`);
        const { data, count, error } = await query;
        if (error)
            return emailError(error.message, 500);
        return emailJson(listEnvelope(data, pagination, count));
    ...
    if (method === "GET" && subscriberId && !action) {
        ...
        if (error)
            return emailError(error.message, 500);
        if (!data)
            return emailError("Subscriber not found", 404);
        return emailJson({ data });
    }
    if (method === "POST" && !subscriberId) {
        ...
        const existing = await supabase
            ...
            .maybeSingle();
        if (existing.error)
            return emailError(existing.error.message, 500);
        ...
        const record = {
            ...
            tags: nextTags,
            workspace,
            status: existing.data?.status === "unsubscribed"
                ? "unsubscribed"
                : parsed.data.status ?? "active",
        };
        const result = existing.data
            ? await supabase.from("subscribers").update(record).eq("id", existing.data.id).select("*").single()
            : await supabase.from("subscribers").insert(record).select("*").single();
        if (result.error)
            return emailError(result.error.message, 500);
...
async function handleTags(request, method, workspace, path, options) {
    const supabase = createEmailSupabaseClient(options);
    const tagId = path[1];
    if (method === "GET" && !tagId) {
        const { data, error } = await supabase
...
async function bulkTag(request, workspace, options, add) {
...
function rangeFor(pagination) {
    return [pagination.offset, pagination.offset + pagination.limit - 1];
}
function listEnvelope(data, pagination, count) {
    return { data: data ?? [], pagination: { ...pagination, count: count ?? 0 } };
}
function parseWorkspace(value, options) {
    return allowedWorkspaces(options).includes(value) ? value : null;
}
function allowedWorkspaces(options) {
