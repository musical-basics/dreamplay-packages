import { createEmailSupabaseClient, emailError, emailJson, type EmailEngineOptions } from "./database.js";

export type TrackingBaseOptions = EmailEngineOptions & {
  fromDomainTrackingBaseUrls?: Record<string, string>;
};

export const DEFAULT_FROM_DOMAIN_TRACKING_BASE_URLS: Record<string, string> = {
...
export function pickTrackingBaseUrl(
  fromEmail: string | null | undefined,
  options: TrackingBaseOptions = {}
): string {
  if (fromEmail) {
    ...
    if (domain && mappings[domain]) return mappings[domain]!;
  }
  return options.trackingBaseUrl ?? process.env.TRACKING_BASE_URL ?? "https://email.dreamplaypianos.com";
}

export function appendEmailAttributionToLinks(
  html: string,
  input: {
    subscriberId: string;
    campaignId: string;
  ...
  return html.replace(/href=(["'])(https?:\/\/[^"']+)\1/g, (match, quote, url) => {
    ...
    if (input.clickTrackingMode === "redirect") {
      const redirectUrl =
        `${input.trackingBaseUrl ?? ""}/api/track/click?c=${encodeURIComponent(input.campaignId)}` +
...
export function injectOpenPixel(
  html: string,
  input: { subscriberId: string; campaignId: string; trackingBaseUrl: string }
): string {
...
export function createEmailOpenTrackingHandler(options: EmailEngineOptions = {}) {
  return async function GET(request: Request): Promise<Response> {
    const url = new URL(request.url);
    const campaignId = url.searchParams.get("c");
    const subscriberId = url.searchParams.get("s");
    if (!campaignId || !subscriberId) return transparentPixel();
...
export function createEmailClickTrackingHandler(options: EmailEngineOptions = {}) {
  return async function GET(request: Request): Promise<Response> {
    const url = new URL(request.url);
    const campaignId = url.searchParams.get("c");
    const subscriberId = url.searchParams.get("s");
...
export function createEmailTrackingHealthHandler(options: EmailEngineOptions = {}) {
  return function GET(): Response {
    const hasSupabase =
      Boolean(options.supabaseUrl ?? process.env.NEXT_PUBLIC_SUPABASE_URL) &&
...
function appendSidCid(url: string, subscriberId: string, campaignId: string): string {
  try {
